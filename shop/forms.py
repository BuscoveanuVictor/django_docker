...
class ContactForm(forms.Form):
    ...
    data_nasterii = forms.DateField(
        widget=forms.DateInput(attrs={'placeholder': 'An-Luna-Zi'}),
        label='Data nasterii',
...
class InstrumentForm(ModelForm):
    ...
    # Campuri aditionale pentru calcularea pretului final
    cost_productie = forms.DecimalField(
        label='Cost de productie (lei)',
        min_value=0,
        error_messages={
            'required': 'Costul de productie este obligatoriu!',
            'min_value': 'Costul de productie nu poate fi negativ!'
        }
    )
    ...
    marja_profit = forms.DecimalField(
        ...
        error_messages={
            ...
            'min_value': 'Marja de profit nu poate fi negativa!',
            'max_value': 'Marja de profit nu poate depasi 100%'
        }
    )
    ...
    def clean_description(self):
        ...
        if len(description) < 50:
            raise forms.ValidationError(
                'Descrierea trebuie sa contina cel putin 50 de caractere!'
            )
        if not description[0].isupper():
            raise forms.ValidationError(
                'Descrierea trebuie sa inceapa cu litera mare!'
            )
        return description
    ...
    def clean_model(self):
        ...
        if not all(char.isalnum() or char.isspace() for char in model):
            raise forms.ValidationError(
                'Numele modelului poate contine doar litere, cifre si spatii!'
            )
        return model
    ...
    def clean(self):
        ...
        if cost_productie and marja_profit:
            if cost_productie < 100 and marja_profit > 50:
                raise forms.ValidationError(
                    'Pentru produse cu cost sub 100 lei, marja de profit nu poate depasi 50%'
                )
        ...
        if stock and cost_productie:
            ...
            if valoare_totala > 10000:
                raise forms.ValidationError(
                    'Valoarea totala a stocului nu poate depasi 10.000 lei!'
                )
    ...
    def save(self, commit=True):
        ...
        # Calculam pretul final folosind costul de producție si marja de profit
        ...
        # Rotunjim la cel mai apropiat numar intreg
        instance.price = Decimal(pret_final).quantize(Decimal('0'))
        
        # Setam un rating initial de 0
        instance.rating = 0
